...
#### This is the function that moves time along
# Change it so that the vectors also move at the same time as humans
def tick(world):
    newTick = world.copy()
    #go through each item/cell in the grid
    for item in world:
        if world[item] == human:   #if human
            newTick[item] = empty    #empty current cell  - NOTE WE ARE NOW USING A NEW FUNCTION FOR MOVE (to replace repetitive code)
            newTick[move(item,RIGHT)] = human
